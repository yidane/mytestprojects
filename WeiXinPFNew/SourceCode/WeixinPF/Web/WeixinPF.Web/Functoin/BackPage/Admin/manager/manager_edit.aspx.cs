    ...
    public partial class manager_edit : ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == MXEnums.ActionEnum.Edit.ToString())
            {
                ...
                if (!new ManagerService(new ManagerRepository(siteConfig.sysdatabaseprefix)).Exists(this.id))
                {
        ...
        private bool DoAdd()
        {
            ...
            model.role_type = new ManagerRoleService(new ManagerRoleRepository(siteConfig.sysdatabaseprefix)).GetModel(model.role_id).role_type;
            ...
            if (addId>0 && isAgent)
            {
                ...
                if (updateRet)
                {
                    ...
                    var bill = new WX_ManagerBillInfo
                    {
                        ...
                        billUsed = "开通1个用户" + model.user_name + "的" + newMaxNum + "个微帐号",
                        moneyType = "扣减"
                    };
            ...
            if (updateRet && isAgent && addNewNum > 0)
            {
                ...
                if (updateRet2)
                {
                    ...
                    var bill = new WX_ManagerBillInfo
                    {
                        ...
                        operPersonId = agent.managerId,
                        operDate = DateTime.Now,
                        billUsed = "原用户" + model.user_name + "新增了" + addNewNum + "个微帐号",
                        moneyType = "扣减"
                    };
                    int addBillId = bBll.Add(bill);
